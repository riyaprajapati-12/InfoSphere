...
import { FiZap, FiCpu, FiGlobe } from "react-icons/fi";

export default function ArticleDetail() {
  const { id } = useParams();
  const [article, setArticle] = useState(null);
  const [summarizing, setSummarizing] = useState(false);
  const [targetLang, setTargetLang] = useState("English"); // Default Language

  const handleGenerateSummary = async () => {
    try {
      setSummarizing(true);
      // Backend ko language bhej rahe hain
      const res = await API.post(`/api/articles/${id}/summarize`, { language: targetLang });
      setArticle((prev) => ({
        ...prev,
        summary: res.data.summary,
        keywords: res.data.keywords
    ...
    <div className="max-w-4xl mx-auto p-6">
      ...
      {!article?.summary && (
        <div className="flex items-center gap-4 mb-6 bg-white/5 p-4 rounded-2xl border border-white/10">
          <div className="text-emerald-400 flex items-center gap-2 font-black text-[10px] uppercase tracking-widest">
            <FiGlobe /> Choose Language:
          </div>
      ...
      {/* --- SUMMARY SECTION --- */}
      <section className="bg-[#161B22] border border-white/10 rounded-[2.5rem] p-8">
        <AnimatePresence mode="wait">
          {article?.summary ? (
            <motion.p className="text-lg leading-relaxed text-slate-300">
              {article.summary}
            </motion.p>
          ) : (
            <div className="text-center">
              <button
                onClick={handleGenerateSummary}
                disabled={summarizing}
                className="flex items-center gap-3 bg-emerald-500 text-black px-8 py-4 rounded-2xl font-black uppercase text-xs mx-auto"
              >
                {summarizing ? <FiCpu className="animate-spin" /> : <FiZap />}
                Generate {targetLang} Summary
              </button>
            </div>
          )}
        </AnimatePresence>
